...
__The fun can start now.__ `r icon::fa("rocket", animate="spin")`
...
ggplot(air_met, aes(y = site_catid, x = Conc)) + 
...
filter(data_2014, SiteNum == 7417) %>% group_by(Date) %>% 
  ...
  ggplot(aes(x = as.Date(Date), y = Conc, na.rm = T)) + 
         ...
         geom_label(aes(x = as.Date("2014-06-11"), y = 0.0527), fill = "#3C989E", 
                    label = "May 25th: the BIG EVENT", size = 7) 
```
...
big_met %>% gather(Parameter, Result, -time, -summary, -icon, -site_catid)
```
---
...
```{r, eval=T, echo=T, message=FALSE, warning=FALSE, out.width="78%"}
# Label our location
leaflet(data = air_sites) %>%
   addTiles() %>%
   addMarkers(label = ~as.character(SiteNum), 
              labelOptions = labelOptions(noHide = T)) %>%
    addPopups(-92.478, 46.6659, "<b><a>You are HERE</a></b>")
``` 

...
.fade[.center[
<img src='http://lazerhorse.org/wp-content/uploads/2013/06/Old-Horror-Films-Retro-Film-Posters-The-Brain-Eaters.jpg' width="88%" style="margin-top: 5%;">
]]
...
<div style="margin-top: -250px; font-size: 80px;">
...
```{r, echo=T, eval=T, message=F, warning=F, out.height="315px", out.width="650px", fig.width=20, fig.height=8}
library(ggplot2)
...
```{r, eval = T, warning=F, message=F, out.height="370px", out.width="1130px", fig.width=28, fig.height=10}
library(ggbeeswarm)

ggplot(pm_data, aes(y = site_catid, x = Conc, fill = site_catid)) + geom_quasirandom(groupOnX = F, alpha = .01, show.legend = FALSE) +   
    labs(title = "PM2.5 Results (ug/m^3)", y = "site ID") #+ xlim(0, 50)
```
...
```{r, echo=T, eval=T, message=F, warning=F, out.height="390px", out.width="800px", fig.width=28, fig.height=12}
...
```{r, echo=F}
theme_set(theme_grey(base_size = 55)) 
```
...
iron(
waffle(filter(pm_sums, site_catid == "27-017-7417") %>% select(aqi_color, count), rows = 7,  size = 0.5, colors = c("#65B345", "#f2d01a"), title  = "Fond du Lac AQI :: 2015"),

waffle(filter(pm_sums, site_catid == "27-137-7549") %>% select(aqi_color, count), rows = 7,  size = 0.5, colors = rev(c("#F26522", "#F2D01A", "#65B345")), xlab = "1 square = 1 day", title  = "Duluth AQI :: 2015"))
```
...
```{r, echo=T, eval=F, fig.width=30, fig.height=11}
...
data_fond <- filter(pm_data, site_catid == "27-017-7417") %>% rename(date = Date)


calendarPlot(data_fond, pollutant  = "Conc", statistic  = "mean", year = 2015, 
             annotate   = "value", digits = 0, key.footer = "ug/m3", 
...
filter(pm_data, site_catid == "27-017-7417") %>% group_by(Date) %>% summarize(Conc = median(Conc, na.rm = T)) %>% 
  ggplot(aes(x = as.Date(Date), y = Conc, na.rm = T)) + 
         geom_line(size = 1, color = "tomato", size = 2) + geom_point(alpha = 0.5, size = 3) + geom_smooth() +
...
ggplot(pm_data, aes(x = Hour, y = Conc)) +
...
> That's nice, but _don't they know_ Excel is limited to around a million rows?
...
.float-left[
![:scale 87%](https://www.kdnuggets.com/wp-content/uploads/hex-dplyr.jpg)
...
__Your analysis toolbox:__ key _dplyr_ functions
...
```{r, echo=F}
library(knitr)
kable(head(fonddulac_met, 4), 'html')
```


---

# Look at correlations with `pairs()`
...
We don't want to take means of data where most of it is missing. Other programs may ignore these values, or throw an error.
...
fonddulac_mean <- filter(air_met, Parameter == 88101) %>% summarize(mean_pm25 = mean(Conc, na.rm = TRUE))

fonddulac_mean
```


Okay, here's the answer _(if we drop all the `NAs`)_.

The average PM2.5 concentration is:  __.scary[.bigtext[`r round(fonddulac_mean$mean_pm25, 2)` _ug/m3_]]__



---

# 11|  R gives you a language for data
...
# 12| Automate - _update with new data_

__Explosions!__

.center[
![:scale 60%](images\explosion.jpg)
]
...
In the mean time you can find help on _R-Bloggers_, download the package _swirl_, and take this [Coursera](https://www.coursera.org/specializations/jhu-data-science) course.
...
<img src="images\business_cat.jpg" align="right" style="margin-left: 40px; margin-right: -28px; margin-top: -10px; max-width: 54%; width: 50%" />

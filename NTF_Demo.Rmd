...
Sometimes we mistakes. You will see us make mistakes. Feel free to laugh at us. It's okay.
...
#### You are air monitoring folks, data analysts, and all around air protectors <span style="font-size: 1.8em;"><i class="fa fa-group" aria-hidden="true" style="color: steelblue;"></i></span> <span style="font-size: 2.7em;">2018</span><span style="font-size: 2em;"><i class="fa fa-group" aria-hidden="true" style="color: steelblue;"></i></span> {-}
...
__It all started with a__ [Pollution Rose](https://shinyapps.io)
...
>
> You are given a lap top with one folder on it. In the folder is an Excel file. On this first day of work you also receive an email asking what you think of the data and that the data isn't a full set and will be updated within the current season. You are told that once the data set is full, you will have 1 day to complete your analysis. 
>
> The __MISSION IMPOSSIBLE__ begins. 


__First things first, let's look at some of the data.__


- __Prep 1:__ _Open the Excel file._


# Get R!

> R is a computer language contributed to and maintained by people all around the world. It lets you perform data analysis like you're writing a recipe for chocolate chip cookies. Once you've written your favorite recipe you can use it over and over again and share it with your collaborators. 
...
> Since this is an Excel file we can load the __readxl__ package to give R the power to read data from Excel.
...
> There are a series of commands within each pacakge. Since we have an `.xlsx` file, we'll use the command `read_excel()`.


```{r, eval=F}
data_2008 <- read_excel("data/example data - firsthalf.xlsx", 
                        sheet = "2008")

```

# Read in the Excel file and look at the tabs.

```{r, eval=F}
data_2009 <- read_excel("data/example data - firsthalf.xlsx", 
sheet = "2009")
```

# Plot the data
> We can look at the data in multiple ways
...
```{r, eval=F}
air_data <- bind_rows(data_2008, data_2009)
```

> Now we've got all the data together, and what dates do we have?

```{r, eval=T}
summary(air_data) ##(can we see the missing dates this way?)
```
...
R is super picky about spelling. We're sorry. It just really wants to be sure it gets your instructions right.
...
> We just learned how to spell lightening storm. I mean __lightning__ storm. 
...
Okay, let's open a new dossier for today's mystery. 

<br>

__Step 1:__ _Start a new project_
...
![](https://assets.wired.com/photos/w_2048/wp-content/uploads/2015/02/catinbox_simba_by-jacme31.jpg)

...
>First day on the job. You are given a lap top with one folder on it. In the folder are two Excel files and the folder is named "PM_and_Ozone". On this first overwhelming day of work you also receive an email asking what you think of the PM and Ozone data. The person on the other side of the phone call also says that the data isn't a full set and will be updated within the current season. Furthermore, the final data set will include measurements near a petroleum refinery explosion and that once you receive it you will have 1 day to complete your analysis of th situation. MISSION IMPOSSIBLE begins (you think briefly about your brother who sells ice cream in California and wonder what that would be like...). 

>First things first, let's load this data set into R.

>R is a language bundled into packages. Packages can be written by anyone, anywhere, which is the power of R. Imagine 1000s of graduate students providing you with tools to analyze and visualize your data, that is exactly what is happening RIGHT NOW.

>In order to complete certain commands, you need to load these packages. So, let's do that first.

> The data are stored in Excel files. So, we will load _readxl_ to pull in the data and complete our mission.


install.packages("readxl")
library(readxl)

>There are a series of commands within each pacakge. Since we've got an xlsx file, we run "read.xlsx()"

data_2008 <- read_xlsx("X:/Agency_Files/Outcomes/Risk_Eval_Air_Mod/_Air_Risk_Evaluation/R/R_Camp/NTP_Training/NTF_learn_R/data/example data - firsthalf.xlsx", sheet = "2008")

##read in the wb and look at the tabs.

data_2009 <- read_xlsx("X:/Agency_Files/Outcomes/Risk_Eval_Air_Mod/_Air_Risk_Evaluation/R/R_Camp/NTP_Training/NTF_learn_R/data/example data - firsthalf.xlsx", sheet = "2009")
...
>Since it looks like our data has the same format and only the year is changing, then we can just bind up the rows with _bind_rows_
...
![](images/name_cloud.png){width=70%}
